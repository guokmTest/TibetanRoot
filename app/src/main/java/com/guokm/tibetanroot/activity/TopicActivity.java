...
public class TopicActivity extends BaseActivity {
    ...
    ListView messageLv;
    private List<Topic> itemList=new ArrayList<Topic>();
...
//                        itemList.add(new String("  ListView item  - add " + page));
                        ...
                        Toast.makeText(TopicActivity.this, "第"+page+"页加载完毕", Toast.LENGTH_SHORT)
                                .show();
                        if (page == 1) {
                            //当为第五页时，停止加载更多
                            //set load more disable
                            ptrFrame_message.setLoadMoreEnable(false);
                            Toast.makeText(TopicActivity.this, "已经全部加载完毕", Toast.LENGTH_SHORT)
                                    .show();
                        }
                        page++;
...
//        Toast.makeText(MessageActivity.this,itemList.get(position).toString(),Toast.LENGTH_SHORT).show();
    ...
    public void getData(int page){
        ...
        for (int i=1;i<21;i++){
            itemList.add(new Topic("这是标题"+i,"2017-08-22"+"第"+(page),"imageurl"));
        }
    }
}
