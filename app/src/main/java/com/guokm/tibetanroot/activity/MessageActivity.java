...
public class MessageActivity extends BaseActivity {
    ...
    ListView messageLv;
    private List<MessageItem> itemList=new ArrayList<MessageItem>();
...
/*        titleLayout = (RelativeLayout) findViewById(R.id.title);
        ...
        final MessageItemAdapter adapter=new MessageItemAdapter(this,itemList);
        messageLv.setAdapter(adapter);
        ptrFrame_message = (PtrClassicFrameLayout) findViewById(R.id.message_list_view_frame);
        ptrFrame_message.setAutoLoadMoreEnable(false);
...
//                        itemList.add(new String("  ListView item  - add " + page));
                        ...
                        Toast.makeText(MessageActivity.this, "第"+page+"页加载完毕", Toast.LENGTH_SHORT)
                                .show();
                        if (page == 1) {
                            //当为第五页时，停止加载更多
                            //set load more disable
                            ptrFrame_message.setLoadMoreEnable(false);
                            Toast.makeText(MessageActivity.this, "已经全部加载完毕", Toast.LENGTH_SHORT)
                                    .show();
                        }
                        page++;
